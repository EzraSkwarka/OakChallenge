  ...
  if (!sameShape) {
    ...
    for (const row of viewModel) {
      ...
      if (tr.parentNode !== tbody) {
        ...
        if (shouldBeAfter && tr.previousSibling !== shouldBeAfter) {
          tbody.insertBefore(tr, shouldBeAfter.nextSibling);
        }
      }
      anchor.node = tr;
    }
    ...
    for (const row of viewModel) {
      if (row.__kind !== "choice") continue;
      const tr = renderCtx.rowNodes.get(row.__key);
      syncChoiceRowState(tr, row);
    }

    exitNodes.forEach(({ key, tr }) => {
      const done = () => {
